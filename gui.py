# -*- coding: utf-8 -*-
# GUI for running VIPER (Voltage Image Processing Execution Routines)
# This file contains the main window and all the tabs for the application.
# Calls to other modules are made to handle specific tasks such as importing data, registration, segmentation, and extraction.
# The GUI is built using PyQt5 and pyqtgraph for visualization.
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setupUi(self, screen_resolution)
    ...
    def openFileDialog(self):
        ...
        if folderPath:
            ...
            # Make sure the 'stacks' directory exists in the output folder
    ...
    def segment_data(self):
        ...
        if self.manualSegmentationFlag:
            ...
            for i in range(len(self.rois)):
                ...
                mask = mask/np.amax(mask)  # Normalize the mask
        ...
        if event[0].double():            
            ...
            for roi in self.rois:
                for handle in roi.getLocalHandlePositions():
                    ...
                    if np.sqrt((img_coords.x() - x)**2 + (img_coords.y() - y)**2) <= 1:
                        self.remove_roi(roi)
                        remove_flag = True
                        break
            
            for coords in self.roi_coords:
                if np.sqrt((img_coords.x() - coords[0])**2 + (img_coords.y() - coords[1])**2) <= 1:
    ...
    def extract_trace(self):
        ...
        registered_data_path = os.path.join(self.tempFolderPath, "registered_data.zarr")
        roi_data_path = os.path.join(self.outputFolderPath,'stacks', "ROIs.tif")
        self.extractionThread = ExtractionThread(registered_data_path, roi_data_path, self.pole, self.frameRate, self.outputFolderPath)
    ...
    def finished_extraction(self, results):
        ...
        c_num,s_num = results['Masks'].shape

        self.traces = np.asarray([object]*s_num, dtype=object)
        self.footprints = np.asarray([object]*s_num, dtype=object)
        self.spikeTimes = np.asarray([object]*s_num, dtype=object)
        self.spikewaveforms = np.asarray([object]*s_num, dtype=object)
        self.spikesnr = np.asarray([object]*s_num, dtype=object)


        for i in range(s_num):
            trace = []
            spike = []
            footprint = np.zeros((results['Masks'][0,i].shape))
            waveform = np.zeros((results['SpikeTemplate'][0,i].shape))
            snr = 0

            for j in range(c_num):
                spike.extend(results['Spikes'][j,i]+len(trace)-1)
                footprint = footprint + results['Masks'][j,i]
                trace.extend(results['DFF'][j,i])
                snr = snr + results['SpikeSNR'][j,i]
                waveform = waveform + results['SpikeTemplate'][j,i]
            
            self.traces[i] = np.array(trace)
            self.footprints[i] = np.array(footprint/c_num)
            self.spikeTimes[i] = np.array(np.array(spike))
            self.spikesnr[i] = snr/c_num
            self.spikewaveforms[i] = waveform/c_num
...
class SaveRegistered(QtCore.QThread):
    ...
    def run(self):
        if os.path.exists(self.outputFolderPath+'/stacks/Registered.tif'):
            ...
            os.remove(self.outputFolderPath+'/stacks/XYshifts.txt')
