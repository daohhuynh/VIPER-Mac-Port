...
if __name__ == "__main__":
    ...
    if (glob.glob(importPath+'/*.tiff')) or (glob.glob(importPath+'/*.tif')):
        ...
        print(f"DEBUG: Loading object of type: {type(rawData)}")
        print(f"DEBUG: Shape found: {rawData.shape}")
        t,n,m = rawData.shape
        ...
        if ds > 1:
            ...
            for i in range(t):
                if gpu_mode:
                    imageData[i,:,:] = cp.asnumpy(downsample2d(cp.array(rawData[i,:,:]), ds, gpu_mode=gpu_mode))
                else:
                    imageData[i,:,:] = downsample2d(rawData[i,:,:], ds, gpu_mode=gpu_mode)
        else:
            imageData = rawData
    ...
    if glob.glob(importPath+'/'+dirs):
        ...
        files = [os.path.join(fname,f) for f in os.listdir(fname) if f.endswith('.tif')]
        files.sort(key=lambda x: int(os.path.basename(x).split('.')[0][1:]))
        item_size = os.path.getsize(os.path.join(fname, files[0]))
            ...
            if len(shape) == 2:
                ...
                for i in range(cores):
                    ...
                    if i < cores-1:
                        ...
                        p = subprocess.Popen([sys.executable, 'VIPER/parallel_load.py', mapped_file_path, str(files_per_core*i), str(files[files_per_core*i:files_per_core*(i+1)]), str(ds), str(int(gpu_mode)), 'frame'])
                        procs.append(p)

                    else:
                        p = subprocess.Popen([sys.executable, 'VIPER/parallel_load.py', mapped_file_path, str(files_per_core*i), str(files[files_per_core*i:]), str(ds), str(int(gpu_mode)), 'frame'])
                        procs.append(p)
                        #procs[i] = multiprocessing.Process(target=map_frames,args=(imageData,files_per_core*i,files[files_per_core*i:], ds, gpu_mode))
                ...
                # For large files, use multiprocessing to map the data
                lengths = np.empty(len(files),dtype=int)
                procs = []
                

                for i in range(len(files)):
                    chunk = tif.TiffFile(os.path.join(fname, files[i]))
                    length = len(chunk.pages)
                    lengths[i] = length

                n,m = chunk.pages[0].shape
                if ds > 1:
                        n = int(n / ds)
                        m = int(m / ds)
                data_type = chunk.pages[0].dtype
                mapped_file_path = os.path.normpath(tempPath + '/raw_map.zarr')
                imageData = zarr.open_array(mapped_file_path, mode='a', shape=(sum(lengths), n, m), chunks=(1, n, m), dtype=data_type)
                start = 0
                for j in range(len(files)):
                    #print(os.path.join(fname, files[j]))
                    #stack = tif.imread(os.path.join(fname, files[j]), aszarr=True)
                    #stack = zarr.open(stack, mode='r')
                    p = subprocess.Popen([sys.executable, 'VIPER/parallel_load.py', mapped_file_path, str(start), os.path.join(fname, files[j]), str(ds), str(int(gpu_mode)), 'chunk'])
                    procs.append(p)
                    start = start + lengths[j]
                        #procs[j] = multiprocessing.Process(target=map_chunks,args=(imageData,start,stack, ds, gpu_mode))
                    # Load frames into mapped file using multiprocessing
                    #if j == 0:
                    #    p = subprocess.Popen([sys.executable, 'VIPER/parallel_load.py', mapped_file_path, str(start), os.path.join(fname, files[j]), str(ds), str(int(gpu_mode)), 'chunk'])
                    #    #procs[j] = multiprocessing.Process(target=map_chunks,args=(imageData,start,stack, ds, gpu_mode))
